...
namespace Little_Uninstall_Manager
{
    public partial class UninstallManager : Form
    {
        ...
        private void PopulateListView()
        {
            ...
            // Get the program info list (HKLM)
            ...
            // HKLM (x64 Wow6432Node registry keys)
            if (Utils.Is64BitOS)
            {
            ...
            // HKCU uninstall keys (per-user applications)
            using (RegistryKey regKey = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall"))
            {
                if (regKey != null)
                {
                    foreach (string strSubKeyName in regKey.GetSubKeyNames())
            ...
            if (Utils.Is64BitOS)
            {
                using (RegistryKey regKey = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall"))
                {
                    if (regKey != null)
                    {
                        foreach (string strSubKeyName in regKey.GetSubKeyNames())
