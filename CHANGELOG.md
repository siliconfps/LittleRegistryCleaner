...
- Final legacy release (pre-modernization base)

## Version 2.0.0 Released

- Upgraded solution and all sub-projects to target .NET Framework 4.0/4.8
- Removed obsolete and missing Microsoft.WindowsAPICodePack dependencies
- Implemented native COM TaskbarProgress (ITaskbarList3) for Windows 7, 8, 10, and 11 taskbar progress support
- Fixed 64-bit handle truncation in xmlRegistry.cs by upgrading P/Invoke signatures to use IntPtr instead of int
- Replaced dangerous reflection in xmlRegistry.cs with RegistryKey.Handle.DangerousGetHandle()
- Fixed unmanaged memory leak in xmlRegistry.cs addRegistryValues with guaranteed try/finally Marshal.FreeHGlobal cleanup
- Protected static root registry hives (HKLM, HKCU, HKCR) from being closed by deleteRegistryKey
- Fixed 64-bit OS detection in Utils.cs using Environment.Is64BitOperatingSystem
- Wrapped all registry key handles across all scanners and utilities in using/try-finally blocks to eliminate handle leaks
- Fixed ActiveX/COM scanner AppID bug checking root CLSID hive instead of the specific CLSID key
- Fixed ApplicationInfo scanner path separator bug and added support for 64-bit Wow6432Node & HKCU uninstall keys
- Fixed ApplicationPaths scanner handle leak and safe type handling for BlockOnTSNonInstallMode
- Fixed ApplicationSettings scanner opening keys as read-only to prevent SecurityException on protected hives
- Fixed potential NullReferenceException in RecentDocs scanner and added modern Windows Shell MUICache support
- Fixed WindowsSounds scanner querying default values with "" instead of "(default)"
- Replaced legacy P/Invoke in WindowsFonts scanner with Environment.SpecialFolder.Fonts and added per-user font support
- Fixed Startup Manager inverted CSIDL icons and folder path mappings between Current User and All Users
- Fixed Startup Manager EditRunItem opening keys with write permissions and safe key disposal
- Fixed Uninstall Manager to scan HKCU uninstall keys and safely close registry keys before deletion
- Fixed Restore dialog and Main form ensuring backup directories exist before reading or writing
- Fixed Windows Forms Application.EnableVisualStyles initialization order and protected EventLog registration
- Cleaned up compiler warnings and resolved missing resource references across all project files
