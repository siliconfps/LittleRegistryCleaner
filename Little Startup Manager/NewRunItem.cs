...
namespace Little_Startup_Manager
{
    public partial class NewRunItem : Form
    {
        ...
        private void buttonOk_Click(object sender, EventArgs e)
        {
            ...
            {
                using (regKey = Registry.LocalMachine.OpenSubKey(strRunRegPath, true))
                {
                    if (regKey != null)
            ...
            {
                using (regKey = Registry.CurrentUser.OpenSubKey(strRunRegPath, true))
                {
                    if (regKey != null)
                        regKey.SetValue(this.textBoxName.Text, strFullPath, RegistryValueKind.String);
                }
            ...
            else if (this.comboBoxSection.Text.StartsWith("StartUp"))
            {
                ...
                if (!string.IsNullOrEmpty(strStartUpPath) && System.IO.Directory.Exists(strStartUpPath))
                {
                    string strShortcutName = this.textBoxName.Text;
                    if (!strShortcutName.EndsWith(".lnk", StringComparison.OrdinalIgnoreCase))
                        strShortcutName += ".lnk";
