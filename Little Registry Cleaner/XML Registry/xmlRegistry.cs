...
namespace Little_Registry_Cleaner.Xml
{
    ...
    public class xmlRegistry
    {
        ...
        [DllImport("advapi32.dll", EntryPoint = "RegOpenKeyA")] public static extern int RegOpenKeyA(IntPtr hKey, string lpSubKey, ref IntPtr phkResult);
        [DllImport("advapi32.dll")] public static extern int RegCloseKey(IntPtr hKey);
        [DllImport("advapi32.dll", EntryPoint = "RegQueryInfoKeyA")] public static extern int RegQueryInfoKeyA(IntPtr hKey, string lpClass, ref int lpcbClass, int lpReserved, ref int lpcSubKeys, ref int lpcbMaxSubKeyLen, ref int lpcbMaxClassLen, ref int lpcValues, ref int lpcbMaxValueNameLen, ref int lpcbMaxValueLen, ref int lpcbSecurityDescriptor, ref System.Runtime.InteropServices.ComTypes.FILETIME lpftLastWriteTime);
        [DllImport("advapi32.dll", EntryPoint = "RegEnumValueA")] public extern static int RegEnumValueA(IntPtr hkey, int index, byte[] lpValueName, ref int lpcbValueName, IntPtr reserved, ref int lpType, IntPtr lpData, ref int lpcbData);
        [DllImport("advapi32.dll", EntryPoint = "RegEnumKeyExA")] public static extern int RegEnumKeyExA(IntPtr hKey, int dwIndex, ref byte lpName, ref int lpcbName, int lpReserved, string lpClass, ref int lpcbClass, ref System.Runtime.InteropServices.ComTypes.FILETIME lpftLastWriteTime);
        [DllImport("advapi32.dll", EntryPoint = "RegSetValueExA")] public static extern int RegSetValueExA(IntPtr hKey, string lpSubKey, int reserved, int dwType, ref byte lpData, int cbData);
        [DllImport("advapi32.dll", EntryPoint = "RegDeleteValueA")] public static extern int RegDeleteValueA(IntPtr hKey, string lpValueName);
        [DllImport("advapi32.dll", EntryPoint = "RegDeleteKeyA")] public static extern int RegDeleteKeyA(IntPtr hKey, string lpSubKey);
        [DllImport("advapi32.dll", EntryPoint = "RegDeleteTreeA")] public static extern int RegDeleteTreeA(IntPtr hKey, string lpSubKey);
        ...
        ArrayList addRegistryValues(IntPtr hKey)
        {
        ...
        bool saveAsXml(xmlWriter w, bool bFakedXml, string strKeyname, IntPtr hKey, string strLimitValue)
        {
            // write key name
            int classLength = 0;
            int cSubKeys = 0;                 // number of subkeys 
        ...
        void LoadAsXml_SetValue(IntPtr hKey, string strName, string strValue, string strType)
        {
        ...
        void DeleteAsXml_DeleteKey(IntPtr hKey, string strName)
        {
            if (hKey == IntPtr.Zero) return;
        ...
        void DeleteAsXml_DeleteValue(IntPtr hKey, string strName)
        {
            if (hKey == IntPtr.Zero) return;
        ...
        void DeleteAsXml_DeleteTree(IntPtr hKey)
        {
            if (hKey == IntPtr.Zero) return;
        ...
        bool deleteRegistryKey(string strBaseKey, string strSubKey, string strLimitValue)
        {
            try
            {
                ...
                else
                {
                    ...
                    else if (upperBase == "HKEY_LOCAL_MACHINE" || upperBase == "HKLM")
                        reg = Registry.LocalMachine;
                    else if (upperBase == "HKEY_USERS" || upperBase == "HKU")
                        reg = Registry.Users;
