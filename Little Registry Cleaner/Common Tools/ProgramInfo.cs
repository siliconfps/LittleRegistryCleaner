...
namespace Common_Tools
{
    public class ProgramInfo : IComparable<ProgramInfo>
    {
        ...
        private void GetARPCache()
        {
            try
            {
                string arpPath = @"SOFTWARE\Microsoft\Windows\CurrentVersion\App Management\ARPCache\" + ParentKeyName;
                RegistryKey regKey = Registry.CurrentUser.OpenSubKey(arpPath);
                if (regKey == null)
                    regKey = Registry.LocalMachine.OpenSubKey(arpPath);

                if (regKey != null)
                {
                    using (regKey)
                    {
                        byte[] b = regKey.GetValue("SlowInfoCache") as byte[];
                        if (b != null && b.Length >= Marshal.SizeOf(typeof(SlowInfoCache)))
                        {
        ...
        public bool RemoveFromRegistry()
        {
            ...
            string strWowKeyName = @"SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\" + Key;

            try
            {
                bool deleted = false;
                using (RegistryKey rk = Registry.LocalMachine.OpenSubKey(strKeyName, false))
                {
                    if (rk != null)
                    {
                ...
                if (!deleted && Environment.Is64BitOperatingSystem)
                {
                    using (RegistryKey rk = Registry.LocalMachine.OpenSubKey(strWowKeyName, false))
                    {
                        if (rk != null)
                        {
                            Registry.LocalMachine.DeleteSubKeyTree(strWowKeyName);
                            deleted = true;
                        }
                    }
                }
                ...
                if (!deleted)
                {
                    using (RegistryKey rk = Registry.CurrentUser.OpenSubKey(strKeyName, false))
                    {
                        if (rk != null)
                        {
                            Registry.CurrentUser.DeleteSubKeyTree(strKeyName);
                            deleted = true;
                        }
                    }
                }
                ...
                if (!deleted && Environment.Is64BitOperatingSystem)
                {
                    using (RegistryKey rk = Registry.CurrentUser.OpenSubKey(strWowKeyName, false))
                    {
                        if (rk != null)
                        {
                            Registry.CurrentUser.DeleteSubKeyTree(strWowKeyName);
                            deleted = true;
                        }
                    }
                }
