    ...
    public class ApplicationPaths : ScannerBase
    {
        ...
        public static void Scan()
        {
            try
            {
                ...
                ScanInstallFolders(Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Installer\Folders"));
                if (Utils.Is64BitOS)
                {
                    ScanInstallFolders(Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Installer\Folders"));
                }

                Main.Logger.WriteLine("Checking for invalid application paths");
                ScanAppPathsKey(Registry.LocalMachine.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\App Paths"));
                ScanAppPathsKey(Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\App Paths"));

                if (Utils.Is64BitOS)
                {
                    ScanAppPathsKey(Registry.LocalMachine.OpenSubKey(@"Software\Wow6432Node\Microsoft\Windows\CurrentVersion\App Paths"));
                    ScanAppPathsKey(Registry.CurrentUser.OpenSubKey(@"Software\Wow6432Node\Microsoft\Windows\CurrentVersion\App Paths"));
                }
            }
        ...
        private static void ScanInstallFolders(RegistryKey regKey)
        {
            if (regKey == null)
                return;
        ...
        private static void ScanAppPathsKey(RegistryKey regKey)
        {
            if (regKey == null)
                return;
            ...
            try
            {
                ...
                {
                    using (RegistryKey regKey2 = regKey.OpenSubKey(strSubKey))
                    {
                        if (regKey2 != null)
                        {
                            ...
                            try
                            {
                                object blockVal = regKey2.GetValue("BlockOnTSNonInstallMode");
                                if (blockVal != null && Convert.ToInt32(blockVal) == 1)
                                    continue;
                            }
                            catch
                            {
