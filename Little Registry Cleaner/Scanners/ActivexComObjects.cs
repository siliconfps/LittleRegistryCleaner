    ...
    public class ActivexComObjects : ScannerBase
    {
        ...
        private static void ScanCLSIDSubKey(RegistryKey regKey)
        {
            ...
            try
            {
                ...
                {
                    using (RegistryKey rkCLSID = regKey.OpenSubKey(strCLSID))
                    {
                        if (rkCLSID == null)
                            continue;
                        ...
                        string strAppID = rkCLSID.GetValue("AppID") as string;
            ...
            if (regKey != Registry.ClassesRoot && regKey != Registry.LocalMachine && regKey != Registry.CurrentUser)
                regKey.Close();
            return;
        }
        ...
        private static void ScanClasses(RegistryKey regKey)
        {
            ...
            try
            {
                foreach (string strSubKey in regKey.GetSubKeyNames())
                {
                    ...
                    if (strSubKey.Length > 0 && strSubKey[0] == '.')
                    {
                        // File Extension
            ...
            try
            {
                ...
                using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\explorer\\Browser Helper Objects"))
                {
                    ...
                    if (regKey != null)
                    {
                        ...
                        {
                            using (RegistryKey rkBHO = regKey.OpenSubKey(strGuid))
                            {
                                if (rkBHO != null)
                                {
                                    // Update scan dialog
                ...
                using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\Internet Explorer\\Extensions"))
                {
                    ...
                    if (regKey != null)
                    {
                        foreach (string strGuid in regKey.GetSubKeyNames())
                        {
                            using (RegistryKey rkExt = regKey.OpenSubKey(strGuid))
                            {
                                if (rkExt != null)
                                {
                                    // Update scan dialog
                ...
                using (RegistryKey regKey = Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\FileExts"))
                {
                    ...
                    if (regKey != null)
                    {
                        ...
                        {
                            if (string.IsNullOrEmpty(strFileExt) || strFileExt[0] != '.')
                                continue;

                            using (RegistryKey rkFileExt = regKey.OpenSubKey(strFileExt))
                            {
                                if (rkFileExt == null)
                                    continue;
        ...
        private static void ValidateFileExt(RegistryKey regKey)
        {
            ...
            try
            {
                ...
                using (RegistryKey rkChoice = regKey.OpenSubKey("UserChoice"))
                {
                    if (rkChoice != null)
                        return;
                }
        ...
        private static bool IEToolbarIsValid(string strGuid)
        {
            ...
            using (RegistryKey rk = Registry.ClassesRoot.OpenSubKey("CLSID\\" + strGuid))
            {
                if (InprocServerExists(rk)) return true;
            }

            using (RegistryKey rk = Registry.LocalMachine.OpenSubKey(@"Software\Classes\CLSID\" + strGuid))
            {
                if (InprocServerExists(rk)) return true;
            }

            using (RegistryKey rk = Registry.CurrentUser.OpenSubKey(@"Software\Classes\CLSID\" + strGuid))
            {
                if (InprocServerExists(rk)) return true;
            }
            ...
            if (Utils.Is64BitOS)
            {
                using (RegistryKey rk = Registry.ClassesRoot.OpenSubKey(@"Wow6432Node\CLSID\" + strGuid))
                {
                    if (InprocServerExists(rk)) return true;
                }

                using (RegistryKey rk = Registry.LocalMachine.OpenSubKey(@"Software\Wow6432Node\Classes\CLSID\" + strGuid))
                {
                    if (InprocServerExists(rk)) return true;
                }

                using (RegistryKey rk = Registry.CurrentUser.OpenSubKey(@"Software\Wow6432Node\Classes\CLSID\" + strGuid))
                {
                    if (InprocServerExists(rk)) return true;
                }
            }
        ...
        private static bool appExists(string appName)
        {
            ...
            finally
            {
                foreach (RegistryKey rk in listRegKeys)
                {
                    if (rk != null && rk != Registry.ClassesRoot && rk != Registry.LocalMachine && rk != Registry.CurrentUser)
                        rk.Close();
                }
            }
