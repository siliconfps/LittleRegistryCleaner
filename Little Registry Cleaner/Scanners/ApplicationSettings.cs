    ...
    public class ApplicationSettings : ScannerBase
    {
        ...
        private static void ScanRegistryKey(RegistryKey baseRegKey)
        {
            ...
            try
            {
                ...
                {
                    // Skip essential system keys, we don't want to mess the system up
                    if (strSubKey.Equals("Microsoft", StringComparison.OrdinalIgnoreCase) ||
                        strSubKey.Equals("Policies", StringComparison.OrdinalIgnoreCase) ||
                        strSubKey.Equals("Classes", StringComparison.OrdinalIgnoreCase) ||
                        strSubKey.Equals("Printers", StringComparison.OrdinalIgnoreCase) ||
                        strSubKey.Equals("Wow6432Node", StringComparison.OrdinalIgnoreCase))
                        continue;

                    using (RegistryKey subKey = baseRegKey.OpenSubKey(strSubKey, false))
                    {
                        if (subKey != null && IsEmptyRegistryKey(subKey))
            ...
            {
                if (baseRegKey != Registry.LocalMachine && baseRegKey != Registry.CurrentUser)
                    baseRegKey.Close();
            }
        }
