...
namespace Little_Registry_Cleaner.Scanners
{
    public class WindowsHelpFiles : ScannerBase
    {
        ...
        public static void Scan()
        {
            try
            {
                using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\HTML Help"))
                {
                    CheckHelpFiles(regKey);
                }

                using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\Help"))
                {
                    CheckHelpFiles(regKey);
                }
                ...
                if (Utils.Is64BitOS)
                {
                    using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows\HTML Help"))
                    {
                        CheckHelpFiles(regKey);
                    }

                    using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows\Help"))
                    {
                        CheckHelpFiles(regKey);
                    }
                }
