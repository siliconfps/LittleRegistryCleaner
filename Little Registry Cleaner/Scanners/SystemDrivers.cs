    ...
    public class SystemDrivers : ScannerBase
    {
        ...
        public static void Scan()
        {
            try
            {
                ...
                {
                    ScanDriversKey(regKey);
                }

                if (Utils.Is64BitOS)
                {
                    using (RegistryKey regKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Wow6432Node\Microsoft\Windows NT\CurrentVersion\Drivers32"))
                    {
                        ScanDriversKey(regKey);
                    }
                }
        ...
        private static void ScanDriversKey(RegistryKey regKey)
        {
            if (regKey == null)
                return;
