...
namespace Little_Registry_Cleaner
{
    public partial class Restore : Form
    {
        ...
        private void Restore_Load(object sender, EventArgs e)
        {
            try
            {
                string backupDir = Properties.Settings.Default.strOptionsBackupDir;
                if (string.IsNullOrEmpty(backupDir))
                {
                    backupDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), @"Little Apps\Little Registry Cleaner\Backups");
                    Properties.Settings.Default.strOptionsBackupDir = backupDir;
                }
                ...
                {
                    if (string.Equals(fi.Extension, ".xml", StringComparison.OrdinalIgnoreCase))
                    {
                        ListViewItem lvi = new ListViewItem(new string[] { fi.Name, fi.CreationTime.ToString(), Utils.ConvertSizeToString(fi.Length) });
